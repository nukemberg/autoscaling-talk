...
| # | Slide | Time | Status | Ticket | Content |
|---|---|---|---|---|---|
| 1 | Autoscaling (title) | — | done | — | Cold open |
| 2 | Design for Max | 1.5m | done | kur (pre-epic) | Design/test for max regardless; USL; scaling down from max not up from min → autoscaling is cost optimization, not scaleout |
| 3 | What Could Possibly Go Wrong | 1.5m | done | kur.1 | Autoscaling ≠ static capacity, it's dynamic; feedback loop → new failure modes; teases 3 war stories |
...
| 11 | Responsible Autoscaling | 6m | done | kur.8 | Design for max first, well-behaved units, right signal, tune the loop, protect upstream — less autoscaling is often the fix |
